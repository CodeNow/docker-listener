...
const DockerEventPublish = (job) => {
  return Promise.try((cb) => {
    ...
    .then((newEvent) => {
      const envs = keypather.get(newEvent, 'inspectData.Config.Env')
      if (!envs) { return newEvent }

      envs.forEach((env) => {
        if (!~env.indexOf('runnable_')) { return }
        const split = env.split('=')
        const key = split[0].split('runnable_')[1]
        const value = split[1]
        newEvent.inspectData.Config.Labels[key] = value
      })
...
DockerEventPublish._inspectImage = (job, newEvent) => {
  ...
  if (!DockerEventPublish._isBuildSuccessfulContainerDeath(job)) {
    return newEvent
  }
...
DockerEventPublish._isBuildSuccessfulContainerDeath = (job) => {
  return keypather.get(job, 'Actor.Attributes.type') === 'image-builder-container' &&
    job.status === 'die' &&
    keypather.get(job, 'Actor.Attributes.exitCode') === '0'
}
