      ...
      .then((event) => {
        ...
        switch (event.status) {
          ...
          case 'die':
            ...
            break
          case 'engine_connect':
            log.info(logData, 'DockerEventPublish - publishing engine_disconnect')
            rabbitmq.publish('docker.engine_connect', event)
            break
          case 'engine_disconnect':
            sinceMap.delete(event.node.Addr)
            log.info(logData, 'DockerEventPublish - publishing engine_disconnect')
            rabbitmq.publish('docker.engine_disconnect', event)
            break
          default:
...
DockerEventPublish._formatEvent = (job) => {
  const dockerEvent = JSON.parse(job.event)
  if (dockerEvent.Type === 'swarm') {
