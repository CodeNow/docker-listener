    ...
    container.inspect(function (err, inspectData) {
      if (err) {
        if (err.statusCode === 404) {
          ...
          log.info(put({ err: err }, logData), 'DockerEventPublish - container not found')
          return cb(fatalErr)
        }
        return cb(err)
      }
      ...
      switch (payload.status) {
        ...
        case 'start':
          ...
          rabbitmq.publish('container.life-cycle.started', payload,
            DockerEventPublish._createRoutingKey(data))
          break
        case 'die':
          if (DockerEventPublish._isUserContainer(payload)) {
...
/**
 * should return true if the dockerEvent is a engine dockerEvent
 */
DockerEventPublish._isEngineEvent = function (dockerEvent) {
  var containerEvent = ['engine_disconnect', 'engine_connect']
  return containerEvent.indexOf(dockerEvent.status) >= 0
}
 ...
 * @param {Object} data formated job
 * @param {String} data.org githubId
 * @param {String} data.ip  of host
 * @return {String} routing key
 */
DockerEventPublish._createRoutingKey = function (data) {
  return [
    data.org,
    data.node.name
  ].join('.')
