...
module.exports = class Swarm extends SwarmClient {
   ...
   */
  getEvents (opts) {
    log.info({ opts: opts }, 'Swarm.prototype.getEvents')
    return this.client.getEventsAsync(opts)
  }
  ...
  testEvent () {
    ...
    return this.client.listContainersAsync({
      filters: {
        state: ['running']
      }
    })
    .then((containers) => {
      ...
      return this.topContainer(testContainerId)
      .then(() => {
        topTimer.stop()
