...
RedisPublisherStream.prototype._write = function (chunk, encoding, callback) {
  ...
  events.enhance(ev, function (err, enhanced) {
    /*jshint maxcomplexity: 900000000000 */
    if (err) { enhanced = ev; }
    ...
    switch (enhanced.status) {
      case 'create':
        ...
        if (isUserContainer(enhanced)) {
          log.info({
            enhanced: enhanced
          }, 'inserting on-instance-container-create task into queue');
          hermesClient.publish('on-instance-container-create', enhanced);
        } else if (isBuildContainer(enhanced)) {
          log.info({
            enhanced: enhanced
          }, 'inserting on-create-start-image-builder-container task into queue');
      ...
      case 'die':
        if (isUserContainer(enhanced)) {
          log.info({
            enhanced: enhanced
          }, 'inserting on-instance-container-die task into queue');
