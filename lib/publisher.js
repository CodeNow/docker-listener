...
RedisPublisherStream.prototype._write = function (chunk, encoding, callback) {
  ...
  events.enhance(ev, function (err, enhanced) {
    ...
    switch (enhanced.status) {
      ...
      case 'start':
        if (isUserContainer(enhanced)) {
          log.info(logData, 'inserting on-instance-container-start task into queue');
          hermesClient.publish('on-instance-container-start', enhanced);
        }
        break;
      case 'pull':
        if (isUserContainer(enhanced)) {
          log.info(logData, 'inserting on-instance-container-pull task into queue');
          hermesClient.publish('on-instance-container-pull', enhanced);
        }
        break;
    }
