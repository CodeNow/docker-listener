...
function RedisPublisherStream() {
  ...
  self._write = function(chunk, encoding, callback) {
    ...
    datadog.inc(ev);
    events.enhance(ev, function(err, event) {
      var channel = process.env.DOCKER_EVENTS_NAMESPACE + event.status;
      redis.publish(channel, JSON.stringify(event), function (err) {
        if (err) {
