...
RabbitPublisherStream.prototype._write = function (chunk, encoding, callback) {
  log.info({ encoding: encoding }, 'RabbitPublisherStream.prototype._write')
  if (!chunk) { return callback() }
  var ev
  try {
    ev = JSON.parse(chunk.toString())
  } catch (err) {
    log.error({ err: err, chunk: chunk }, '_write: error parsing')
    return callback(err)
  }
  if (isBlacklisted(ev)) {
    // do nothing
    log.trace({ event: ev }, '_write: event blacklisted')
    return callback(null)
  }
  log.trace({ event: ev }, '_write: event')
