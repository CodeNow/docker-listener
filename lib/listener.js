 ...
 * This should emit the `top` event. We ignore errors since we will restart if event failed
...
Listener.prototype.testEvent = function (cb) {
  ...
  docker.listContainers({
    ...
    // ignore error here, we will just restart if this failed
    log.trace({ container: containers[0] }, 'testEvent - got container')
    containers[0].top(function (err) {
      if (err) {
