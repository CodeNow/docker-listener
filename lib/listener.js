...
module.exports = class EventListener {
  ...
  setTimeout () {
    return new Promise((resolve, reject) => {
      ...
      this.eventStream.once('data', () => resolve())
    })
  }
  /**
  ...
  publishEvent (event) {
    ...
    this.log.info({ event: event.toString() }, 'Listener.prototype.publishEvent')
    try {
      JSON.parse(event)
    } catch (err) {
      return this.log.error({ err: err }, 'publishEvent - error parsing logs')
    }
  ...
  formatEvent (event) {
    ...
    this.log.info({ event: event }, 'Listener.prototype.formatEvent')
    return event
  }
