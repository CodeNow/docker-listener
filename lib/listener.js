...
exports.start = function (publisher, reporter) {
  ...
  docker.getEvents(function (err, eventStream) {
    ...
    if (eventStream) {
      ...
      eventStream.pipe(dogstatsd.stream(), {end: false});
    }
...
function handleConnected (publisher) {
  ...
  dogstatsd.increment('docker.listener.connect');
}
...
function handleError () {
  return function (err) {
    ...
    dogstatsd.increment('docker.listener.error');
  };
}
...
function reconnect (publisher, reporter) {
  if (process.env.AUTO_RECONNECT === 'true') {
    setTimeout(function () {
      ...
      dogstatsd.increment('docker.listener.reconnect');
