...
Listener.prototype.start = function (cb) {
  ...
  docker.getEvents(function (err, eventStream) {
    ...
    eventStream
      ...
      .once('data', function () {
        log.info('start - event data received')
...
Listener.prototype.testEvent = function (cb) {
  ...
  docker.listContainers({
    ...
    if (isEmpty(containers)) {
      ...
      errorCat.createAndReport(404, 'no running containers found')
      return cb()
    }
    ...
    docker.getContainer(containers[0].Id).top(function (err) {
      topTimer.stop()
      if (err) {
        log.error({ container: containers[0], err: err }, 'testEvent - failed to run top')
 ...
 */
Listener.prototype.publishEvent = function (chunk) {
  if (!chunk) { return }
  ...
  rabbitmq.createPublishJob(chunk)
}
 ...
 */
Listener.prototype.handleClose = function (err) {
  log.info('Listener.prototype.handleClose')
  errorCat.createAndReport(500, 'Cannot connect to Docker /events', err, function () {
    process.exit(1)
  })
}
