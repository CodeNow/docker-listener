...
// send `docker.events-stream.connected` event
Listener.prototype.handleConnected = function () {
  log.info('connected to docker');
  var dockerUpEvent = JSON.stringify(events.createEvent('docker.events-stream.connected'));
  datadog.inc('docker.events-stream.connected');
...
Listener.prototype.handleClose = function () {
  ...
  this.dockerEventStream.destroy();
  this.dockerEventStream = null;
  var dockerDownEvent = JSON.stringify(events.createEvent('docker.events-stream.disconnected'));
  datadog.inc('docker.events-stream.disconnected');
