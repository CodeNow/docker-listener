...
module.exports = class EventListener {
   ...
   * 1. start timeout
   * 2. if we get data, clear timeout
   * @returns {Promise}
   * @resolve {undefined} when stream connected
   * @rejects {Error}     when stream failed to get event in time
   */
  testStream () {
    const log = this.log.child({ method: 'testStream' })
    log.trace('start stream test')
    return Promise.fromCallback((cb) => {
      this.eventStream
        .once('data', () => {
          log.trace('event data received, clear timeout')
