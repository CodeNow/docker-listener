 ...
 * @reporter should be used if you want to know when problems happened. It is an optional parameter. It should be writable stream
 */
exports.start = function (publisher, reporter) {
  if (publisher) {
    if (!publisher.writable && !publisher.write) {
      throw new Error('Publisher should be Writable');
    }
  }
  if (reporter) {
    if (!reporter.writable && !reporter.write) {
  ...
  docker.getEvents(function (err, eventStream) {
    ...
    if (eventStream) {
      ...
      eventStream
        ...
        .on('close', function () {
          // TODO (anton) write to the publisher
